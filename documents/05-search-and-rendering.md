...
> ※ この 0.406 秒のほとんどは、Step 3 で入れた**学習用の遅延**（`sleep(400)`）。
> Step 6 でこれを削除すると `/blog` は **約 0.03 秒**になる。
> 動的レンダリング自体のコストはもっと小さい、という点は誤解しないでほしい。
...
→ 次は **[Step 6: SQLite に保存する](./06-drizzle-sqlite.md)**
